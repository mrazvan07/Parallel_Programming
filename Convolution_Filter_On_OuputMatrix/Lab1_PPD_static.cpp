...
using namespace std;
#define MAX_MATRIX_SIZE 10004
#define SMALL_KERNEL_SIZE 3
#define BIG_KERNEL_SIZE 5

int matrix[MAX_MATRIX_SIZE][MAX_MATRIX_SIZE];
int output_matrix[MAX_MATRIX_SIZE][MAX_MATRIX_SIZE];
int kernel[BIG_KERNEL_SIZE][BIG_KERNEL_SIZE];
...
void print_kernel(int matrix[][BIG_KERNEL_SIZE], int rows, int cols) {
...
void print_matrix(int matrix[][MAX_MATRIX_SIZE], int rows, int cols) {
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            cout << matrix[i][j] << " ";
        }
...
void borderMatrix(int matrix[][MAX_MATRIX_SIZE], int kernel_size, int rows, int cols) {
    //K=3
    if (kernel_size == 3) {
...
int transform_pixel(int i, int j) {
    int new_pixel_val = 0;
    int* k_values;
    int* l_values;
    if (kernel_size == 3) {
    ...
    if (byRows) {
        ...
        for (int i = start; i < end; i++) {
            for (int j = start_col_count; j < m + start_col_count; j++) {
                output_matrix[current_row_in_output_matrix][current_col_in_output_matrix] = transform_pixel(i, j);
...
int main(int argc, char* argv[]) {
    ...
    //print_matrix(output_matrix, n, m);
